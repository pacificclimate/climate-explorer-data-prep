...
def main(args):
    filepaths = {"pr": args.prec, "tasmin": args.tasmin, "tasmax": args.tasmax}
    if args.dry_run:
        dry_run(filepaths)
    else:
...
def runme():
    parser = ArgumentParser(
        description="Create precipitation as snow data from pr, tasmin, tasmax"
    )
    ...
    parser.add_argument(
        "-p", "--prec", required=True, help="Precipitation file to process"
    )
