...
import time
'''
Formats a netCDF file to meet the CF Standards for Discrete Structured
Geometry data. This is data recorded at one or more distinct locations,
instead of at every point on a grid.
...
logger.setLevel(logging.DEBUG)  # overriden by -l

# TODO: make clear with variable names which functions accept a variable
#    objects and which the name of one. Better yet, be more consistent
#    about it, pass only what's needed.
...
def format_dsg_timeseries(outdir, input_file, metadata_files,
    ...
    Needed to meet DSG Standards:
        ...
        * Give data variables an attribute with list of instance variables
    ...
    (Though not for other datasets). This script refers to "instance
    variables", but nchelpers calls them "coordinate variables".
    '''
    ...
    try:
        ...
        output = None if dry_run else Dataset(random_filename, "w",
                                              format="NETCDF4")

        # First, copy over dimensions. Don't copy any string length
        # dimensions; we'll be converting character variables to
        # string variables.
        if output:
        ...
        # sampling geometry.
        # This variable is indicated with the cf_role attribute. It is
        # required by the CF standards for discrete sampling geometries.
        # The user may specify which variable it is, it may be identified by
        # the cf_role attribute in the input file, or - as a last resort -
        # the script will check whether there's only one possibility.
        if not id_var:
        ...
        # Check for missing metadata required by the PCIC standards, generate
        # it from RVIC equivalents if possible
        ...
        if output:
            output.history = hist_entry + (
                output.history if "history" in output.__dict__ else ""
                )

    except Exception as e:
        # something went wrong - clean up the files.
        input.close()
        if output:
            logger.info("Cleaning up files after error. No output written")
            output.close()
...
def guess_id_variable(nc, instance_dim):
    ...
    if len(variables) == 0:
        ...
        raise Exception("Multiple variables have a cf_role metadata attribute: {}".format(
            variables))
...
def text_dimension(var):
    ...
    for d in var.dimensions:
        if guess_dim_type(d) == 'text':
            if text_d:
                raise Exception("Error: can't handle multiple text dimensions on variable {}".format(
                    var.name))
            elif var.dtype != 'S1':
                raise Exception("Error: don't understand non-character variable with string length: {}".format(
                    var.name))
            elif len(var.dimensions) == 1:
                raise Exception("Cannot reduce 1-dimensional character variable {}".format(
                    var.name))
            else:
                text_d = d
    return text_d
...
def creation_date_from_history(hist):
    ...
    date = re.search(r'Created: (Sun|Mon|Tue|Wed|Thu|Fri|Sat) (.{3} \d{2} \d{2}:\d{2}:\d{2} \d{4})',
                     hist)
    if date:
...
def find_time_variable(input):
    def is_time_variable(var):
        return (
            len(input.variables[var].dimensions) == 1 and
            guess_dim_type(input.variables[var].dimensions[0]) == 'T'
            )
    return find_singular_item(input.variables, is_time_variable,
                              "time variable", False)
...
def copy_global_metadata(input, prefix, dest):
    ...
    for m in input.__dict__:
        ...
        if prefixed in attributes_list:
            logger.warning("Output file already has a {} attribute".format(
                prefixed))
            copied = copied - 1
        elif testing:
...
def copy_time_metadata(src, dest):
    ...
    if not len(src_time) == len(dest_time):
        raise Exception("Cannot compare time metadata if time dimension is unequal {} {}".format(
            dest.filepath, src.filepath))
    if not equivalent_calendars(src_time.getncattr('calendar'),
                                dest_time.getncattr('calendar')):
    ...
    if ref_date_match:
        ...
        if time:
            ...
            refdate = datetime.datetime(int(ref_date_match.group(1)),
                                        int(ref_date_match.group(2)),
                                        int(ref_date_match.group(3)))
        dest_time.setncattr('units',
                            'days since {}'.format(
                                refdate.strftime("%Y-%m-%d")
                                ))
        dest_time[:] = src_time[:]
    else:
        raise Exception("Cannot understand format of units string: {}".format(
            src_time.getncattr('units')))
...
def rvic_metadata_to_pcic_metadata(input, output, id_var, metadata_files):
    ...
    # Fix time units
    # For inscrutable reasons, RVIC streamflow files are generated with
    ...
    if input.variables[time_var].units in zeroed_time_units:
        ...
        if is_rvic_output(input):
            ...
            hydromodel_file = find_singular_item(metadata_files,
                                                 is_hydromodel_input,
                                                 "hydromodel input file",
                                                 False)
            if isinstance(hydromodel_file, list):
                ...
                if output:
                    try:
                        ...
                        logger.warn("Unable to correct time variable metadata. Reason: {}".format(e))
                hf.close()
            else:
                logger.warn("Unable to determine hydromodel input file "
                            "to update time metadata")
        else:
            logger.warn("Cannot correct time units: {}".format(
                input.variables[time_var].units))

    # PCIC metadata standards require a creation date; parse it
    # from RVIC's 'history' metadata attribute.
    ...
    if 'casename' in input.__dict__:
        ...
        if 'domain' not in input.__dict__:
            logger.info("Generating domain from casename: {}".format(
                casename))
            if output:
                output.setncattr('domain', casename)
        # the default outlet names generated by RVIC are unhelpfully 'p-0',
        # 'p-1', etc. If the outlet name is 'p-0' and this file contains only
        # a single outlet, set the outlet name equal to the casename.
        if (
            'outlet_name' in input.variables and
            id_var == 'outlet_name' and
           'outlets' in input.dimensions and
           len(input.dimensions['outlets']) == 1
           ):
            if output.variables['outlet_name'][0] == 'p-0':
                logger.info("Setting outlet name to casename {}".format(
                    casename))
                if output:
...
def write_variable(input, output, var, id_variable, instance_dim):
    '''Copies a variable from the input netCDF to the output netCDF, making
    the following three changes as necessary:
    ...
    if reduce_dimension:
        ...
        if output:
            ...
            # remove filler characters from fixed-length strings
            strings = [s.rstrip('\x00') for s in strings]
            # match new shape
            shape = tuple([len(input.dimensions[d]) for d in new_dimensions])
            strings = numpy.reshape(strings, shape)
            output.variables[var][:] = strings
    else:
        logger.info("Writing variable {}".format(var))
        if output:
            output.createVariable(var, input.variables[var].dtype,
                                  input.variables[var].dimensions)
